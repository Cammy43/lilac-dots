...
local isGameMode = readFile("/home/" .. arg[1] .. "/.config/hypr/lilac/kv/gamemode");
if isGameMode == "1" then
...
if readFile("/home/" .. arg[1] .. "/.config/hypr/lilac/kv/lilacPM/charging.txt") == "1" then
    ...
    if batPct < 15 then
        if currentMode ~= "powersaver" then
            notify("Eco mode");
        end;
         os.execute("tlpctl power-saver");
        writeFile(currentModePath, "powersaver");
    else
